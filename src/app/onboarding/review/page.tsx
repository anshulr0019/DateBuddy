      ...
      <div className="relative h-full w-full max-w-[440px] sm:max-w-lg md:max-w-xl flex flex-col bg-infyn-paper shadow-2xl sm:border-x sm:border-infyn-ink/5 overflow-hidden">
        ...
        <div className="flex-shrink-0 z-20 px-6 pt-[calc(1.25rem+env(safe-area-inset-top,0px))] pb-2">
          <div className="flex items-center gap-3 mb-3">
            ...
            >
              <div className="h-full w-full rounded-full bg-gradient-to-r from-infyn-rose to-infyn-rose shadow-[0_0_8px_rgba(32,26,22,0.16)]" />
            </div>
        ...
        <div className="flex-1 min-h-0 z-10 px-6 overflow-y-auto scrollbar-none pb-6 animate-page-enter">
          <div className="mb-6">
            <h1 className="text-[28px] font-normal text-infyn-ink tracking-tight leading-[1.1] font-display">
              Looking good! 🔥
            </h1>
            <p className="text-[14.5px] text-infyn-ink/55 mt-1.5">Here&apos;s your profile in Discover</p>
          </div>

          {/* Profile preview card */}
          {profile && (
            <div className="rounded-[24px] border border-infyn-border/80 bg-infyn-surface/80 shadow-[0_10px_30px_-15px_rgba(32,26,22,0.12)] backdrop-blur-md mb-4 overflow-hidden">
              {/* Photo */}
              ...
              <div className="p-4">
                {profile.bio && (
                  <p className="text-[14px] font-medium text-infyn-ink/75 mb-3 leading-relaxed">{profile.bio}</p>
                )}
                {profile.interests.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {profile.interests.map((i) => (
                      <span key={i} className="rounded-full bg-infyn-rose/10 border border-infyn-rose/20 px-3 py-1 text-[12px] font-semibold text-infyn-rose">
                        {i}
                      </span>
                    ))}
                  </div>
          ...
          {/* Profile strength */}
          <div className="rounded-[24px] border border-infyn-border/80 bg-infyn-surface/80 p-4 shadow-[0_4px_20px_-10px_rgba(32,26,22,0.08)] backdrop-blur-md flex items-center gap-4">
              ...
              <svg className="-rotate-90 h-16 w-16" viewBox="0 0 64 64" aria-hidden>
                ...
                <defs>
                  <linearGradient id="rv-grad" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="var(--infyn-rose)" />
                    <stop offset="100%" stopColor="var(--infyn-rose)" />
                  </linearGradient>
                </defs>
              </svg>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[14px] font-black text-infyn-ink">{strengthPct}%</span>
              </div>
            </div>
            <div>
              <p className="text-[15px] font-bold text-infyn-ink">Profile Strength</p>
              <p className="text-[12.5px] text-infyn-ink/55 mt-0.5 leading-snug">
        ...
        <div className="flex-shrink-0 z-20 px-6 pt-4 pb-[calc(1.25rem+env(safe-area-inset-bottom,0px))] bg-gradient-to-t from-infyn-paper via-infyn-paper/90 to-transparent border-t border-black/[0.04] space-y-2.5">
          ...
          <button
            ...
            disabled={submitting}
            className="w-full h-14 rounded-2xl bg-gradient-to-r from-infyn-rose to-infyn-rose text-white text-[15px] font-bold shadow-[0_10px_28px_-8px_rgba(32,26,22,0.16)] active:scale-[0.985] transition-all cursor-pointer flex items-center justify-center gap-2 disabled:opacity-60 disabled:active:scale-100"
          >
            {submitting ? (
              <>
          ...
          <button
            ...
            disabled={submitting}
            className="w-full h-12 rounded-2xl border border-infyn-ink/10 bg-infyn-surface/80 text-infyn-ink text-[13.5px] font-bold hover:bg-infyn-surface transition-all cursor-pointer shadow-sm disabled:opacity-50 active:scale-[0.98]"
          >
