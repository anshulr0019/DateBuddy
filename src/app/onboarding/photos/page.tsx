...
export default function PhotosPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!rawFile || activeSlot === null) return;
    const targetIndex = activeSlot;
    setUploadingIndex(targetIndex);
    setError('');
    try {
      // Compress large high-res camera photos on device before uploading
      ...
      <div className="relative h-full w-full max-w-[440px] sm:max-w-lg md:max-w-xl flex flex-col bg-infyn-paper shadow-2xl sm:border-x sm:border-infyn-ink/5 overflow-hidden">
        ...
        <div className="flex-shrink-0 z-20 px-6 pt-[calc(1.25rem+env(safe-area-inset-top,0px))] pb-2">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-[12px] font-bold tabular-nums text-infyn-ink/45">3 of 7</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 z-10 px-6 overflow-y-auto overflow-x-hidden overscroll-contain touch-pan-y scrollbar-none pb-8 [-webkit-overflow-scrolling:touch]">
          ...
          <div className="mb-6">
            <h1 className="text-[28px] font-black text-infyn-ink tracking-tight leading-[1.1]">Add your best photos</h1>
            <p className="text-[14.5px] text-infyn-ink/55 mt-1.5">At least 2 photos · Profiles with 4+ photos get 3× more matches</p>
          </div>
                ...
                <div key={index} className="relative">
                  <button
                    ...
                    className={`aspect-[3/4] w-full rounded-[22px] overflow-hidden transition-all duration-200 cursor-pointer ${
                      url
                        ? 'shadow-[0_8px_24px_-8px_rgba(32,26,22,0.16)]'
                        : `border-2 border-dashed flex flex-col items-center justify-center ${
                            required
                              ? 'border-infyn-rose/40 bg-infyn-rose/[0.04] hover:border-infyn-rose/70 hover:bg-infyn-rose/[0.08]'
                              : 'border-infyn-ink/15 bg-infyn-surface/60 hover:border-infyn-ink/30 hover:bg-infyn-surface/80'
                          }`
                    }`}
                    style={url ? { backgroundImage: `url(${url})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}
                  >
                    {uploading && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center bg-infyn-surface/85 backdrop-blur-sm rounded-[22px]">
                        <svg className="animate-spin mb-1.5" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--infyn-rose)" strokeWidth="2.5" strokeLinecap="round">
                          <path d="M21 12a9 9 0 1 1-6.2-8.56" />
                        </svg>
                        <span className="text-[11px] font-bold text-infyn-rose">Uploading…</span>
                      </div>
                    )}
                    {!url && !uploading && (
                      <div className="flex flex-col items-center text-center p-3">
                        <div className={`flex h-11 w-11 items-center justify-center rounded-full mb-1.5 ${required ? 'bg-infyn-rose/15' : 'bg-infyn-ink/[0.06]'}`}>
                        ...
                        <span className={`text-[10.5px] font-bold uppercase tracking-wider ${required ? 'text-infyn-rose' : 'text-infyn-ink/35'}`}>
                  ...
                  {url && !uploading && (
                    <button
                      ...
                      className="absolute top-2 right-2 flex h-7 w-7 items-center justify-center rounded-full bg-infyn-surface/90 border border-infyn-border/60 text-infyn-ink/70 shadow-md hover:bg-infyn-surface active:scale-90 transition-all cursor-pointer backdrop-blur-md"
                    >
            ...
            {[0, 1, 2, 3, 4, 5].map(i => (
              <div
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  photos[i] ? 'w-5 bg-gradient-to-r from-infyn-rose to-infyn-rose' : 'w-1.5 bg-infyn-ink/15'
                }`}
              />
            ))}
          </div>
          <p className="mt-2 text-center text-[12.5px] font-medium text-infyn-ink/45">
            {validCount} of 6 photos added
            {validCount < 2 && (
              <span className="text-infyn-rose font-bold"> · Add {2 - validCount} more to continue</span>
            )}
          </p>
        </div>
